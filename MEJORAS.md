...
- _(Post-P1)_ Perfiles locales (81 asserts). IndexedDB schema bump v1→v2: nuevo almacén `profiles` (`{id, name, pinHash, createdAt, isGuest?}`) + índice `profileId` en `games`. Migración auto-crea perfil "Invitado" y asigna las partidas legacy. **PIN obligatorio de 4 dígitos en la creación** (con campo de confirmación para evitar tipearlo mal), hasheado con SHA-256 vía `crypto.subtle.digest`. Bloqueo de 30s tras 3 intentos fallidos (estado en memoria). Pantalla a pantalla completa al cargar bloquea el juego hasta elegir o crear perfil; el PIN se vuelve a pedir en cada arranque (sin sesión persistente). Botón `👤 nombre` en la barra de controles abre menú con editar/cambiar perfil/borrar. `buildGameRecord` asigna `profileId` y usa nombre del perfil en `white`/`black` para 1p y LAN. `lanOnDCOpen` envía el nombre del perfil en el `hello`. El PIN se hashea con SHA-256: en contexto seguro vía `crypto.subtle` (Web Crypto), y sobre HTTP por IP de LAN cruda —donde `crypto.subtle` no existe, p. ej. el celular en `http://192.168.x.x`— cae a un **SHA-256 en JS puro** que produce un hash idéntico (`_sha256HexBytes`, verificado contra el estándar). Así la creación y verificación del PIN funcionan en cualquier contexto (HTTPS, `localhost`, `file://` o HTTP por IP).
- _(Post-P1)_ Colección "♛ Partidas Célebres" (Clásicas): 123 partidas inmortales de la historia (1820–2006: el Inmortal de Anderssen, la Siempreviva, la Opera Game de Morphy, la Partida del Siglo de Fischer, el Inmortal de Kasparov, Rotlewi–Rubinstein, Steinitz–von Bardeleben, etc.). Embebidas como movetext SAN compacto (~44 KB) en `CLASSIC_GAMES` y expandidas con `pgnToGameRecord` al abrir cada una (parseo perezoso). Las partidas fueron verificadas jugada por jugada contra el motor (round-trip 123/123 tras retirar una transcripción duplicada de la Inmortal). Botón "♛ Clásicas" abre un modal con buscador (por jugador/evento/año) y tabla; "👁 Ver" abre la partida en el visor de estudio vía `viewGameRecord`. Colección de **solo lectura**: no se guardan en IndexedDB ni dependen del perfil activo. Movimientos tomados de colecciones públicas de _immortal games_ (las jugadas de ajedrez son hechos, sin copyright).
- _(Post-P1)_ Narración dramática de Clásicas (las 123 partidas con guión completo). Botón "🔊 Narrar" en el visor de estudio (solo aparece si la partida tiene guión). Usa el lector de voz integrado del navegador (Web Speech API / `speechSynthesis`) — **100% offline, cero dependencias y cero archivos de audio**: el guión es texto embebido en `CLASSIC_NARRATION` (con clave `narrationKey`, un hash de las jugadas) y la voz la pone el sistema del usuario. `pickSpanishVoice()` prefiere una voz **local** en español (es-MX > es-US/es-419 > es-\*) para garantizar que no haya tráfico de red. La reproducción narrada arranca en la posición inicial y avanza **jugada por jugada**, una pieza a la vez con un deslizamiento suave (clon flotante que reutiliza el estilo `.drag-overlay` del arrastre); se detiene a narrar en los plies con guión —las piezas quedan inmóviles mientras habla la voz— y reanuda al terminar, hasta el final de la partida (`advanceNarration` + `narrateAtPly` + `animateReplayMove`). El ritmo de las jugadas en silencio se mantiene ágil (~490 ms/jugada) y los guiones intercalan líneas puente en los huecos largos, de modo que ningún silencio entre narraciones supere ~3.5 s. Ritmo (`rate`) más lento en los momentos clímax (sacrificios de alfil y de dama). Si el usuario navega manualmente, la narración se detiene sola (guard en `replayGoto` con la bandera `narrationDriving`); también se detiene al salir del visor. Los guiones se escriben por lotes para toda la colección (anclados a las jugadas reales, con `rate` más lento en los clímax) y se van quedando listos para una voz premium futura. La clave `narrationKey` es un hash de las jugadas (evita colisiones entre partidas con mismos jugadores/año). En la lista, las partidas con guión muestran 🔊.
- _(Post-P1)_ Conexión LAN automática (sin copiar/pegar). Nuevo `chess-lan-server.py` (solo biblioteca estándar de Python): sirve los archivos **y** ofrece señalización WebRTC por HTTP (`/lan/ping`, `/lan/offer`, `/lan/answer`, con salas que expiran a los 10 min). Al abrir 📡 LAN, el modal hace `GET /lan/ping`: si hay servidor de emparejamiento, muestra "Crear sala" / "Unirse a sala" (el anfitrión obtiene un código de 4 dígitos; el invitado lo escribe y la conexión se establece sola); si no, cae al **modo manual** copiar/pegar de siempre (fallback para `file://` o `http.server`). Reutiliza toda la maquinaria WebRTC existente (`lanCreateOffer`/`lanCreateAnswer` + sondeo `lanPollForAnswer`); las jugadas siguen viajando **cifradas de extremo a extremo** por el canal WebRTC (DTLS) — el servidor solo ve el SDP del saludo. Seguridad: el servidor se ata a la LAN (no exponer a internet) y el código de sala evita emparejamientos accidentales. Esto resuelve además que sobre `http://IP-LAN` el portapapeles y la cámara estén bloqueados (contexto inseguro), que era lo que hacía penoso el copy-paste manual.
...
- Dos dispositivos conectados a la **misma red WiFi/LAN**.
- Navegador moderno (Chrome 86+ / Firefox 78+ / Safari 14+ / Edge 86+ / Chrome Android 88+ / Safari iOS 14.5+).
- **No requiere internet** — funciona entre dos dispositivos en la misma red aislada.

### Modo recomendado: conexión automática (con `chess-lan-server.py`)

1. En la máquina anfitriona, corre: `python3 chess-lan-server.py` (escucha en `0.0.0.0:8000` — toda la LAN). La consola muestra la IP a compartir, p. ej. `http://192.168.1.50:8000`.
2. El anfitrión abre `http://localhost:8000`, va a **📡 LAN** → **Crear sala**. Aparece un **código de 4 dígitos**.
3. El invitado abre **`http://IP-DEL-ANFITRION:8000`** en su navegador → **📡 LAN** → **Unirse a sala**, escribe el código y **✓ Conectar**.
4. La conexión se establece sola (sin copiar/pegar). Las jugadas viajan cifradas de extremo a extremo por WebRTC.

> Seguridad: el servidor se ata a la red local; no lo expongas a internet. El código de sala evita emparejamientos accidentales.

### Modo manual (fallback, sin servidor de emparejamiento)

Si abres el `index.html` como `file://` o con un servidor sin señalización, el modal LAN muestra el flujo manual de copiar/pegar el SDP. Pasos:
