...
- _(Post-P1)_ Perfiles locales con PIN opcional (68 asserts). IndexedDB schema bump v1→v2: nuevo store `profiles` (`{id, name, pinHash?, createdAt, isGuest?}`) + índice `profileId` en `games`. Migración auto-crea perfil "Invitado" y asigna las games legacy. PIN vía SHA-256 (`crypto.subtle.digest`) en contextos seguros con lockout de 30s tras 3 intentos fallidos. Pantalla full-screen al cargar bloquea el juego hasta elegir o crear perfil. Botón `👤 nombre` en barra de controles abre menú con editar/cambiar perfil/borrar. `buildGameRecord` asigna `profileId` y usa nombre del perfil en `white`/`black` para 1p y LAN. `lanOnDCOpen` envía el nombre del perfil en el `hello`. PIN-protected re-prompt en cada arranque (sin sesión persistente).
